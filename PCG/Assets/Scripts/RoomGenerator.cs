...
public class RoomGenerator : MonoBehaviour {
    ...
    //Room size distribution - Limiting the size of each room so there isn't very narrow or very big ones
    ...
    public void Generate(int roomCount, int radius, float mainRoomFrequency, float roomConnectionFrequency)
    {
        ...
        //Initializing my rooms in the way I want them :P
        ...
        {
            //Calculate which rooms are gonna be main ones - They have to be the largest ones
    ...
    //Random position to spawn a room
    ...
    void Update()
    {
        ...
        //Check if all physics objects are done moving
        if (allSleeping && !Done)
        {
            Done = true;
            Calculate();
    ...
    //Create the smallest path possible between all main rooms
    ...
    //In order for the dungeon to look interesting, I will add more connections to our minimum spanning tree. Hey the dungeon lord asked for several escape routes, you know for when shit hits the fan xD
            ...
            if (Rooms[n].IsMainRoom)
            {
                ...
                for (int i = 0; i < Rooms[n].Connections.Count; i++)
                {
                    ...
                    if (direction == ConnectionType.Up)
                    {
                        ...
                        p3 = p2;
                        //For some reason hallways are off by 3 pixels in this direction only, after some time trying to figure out I gave up....
                        //it works fine just adding 3 units
                        if (p0.x > p1.x)
                        {
    ...
    void Calculate()
    {
        ...
        //Get a point list of all main rooms
        for (int n = 0; n < Rooms.Count; n++)
        {
            if (Rooms[n].IsMainRoom)
            {
